...
public class GridShadowCastersGenerator : MonoBehaviour {
    ...
    public GameObject[] Generate() {
        ...
        for (int y = 0; y < countY; y++) {
            for (int x = 0; x < countX; x++) {
                hits[x, y] = IsHit(new Vector2(bottomLeft.x + (x*ratio) + (0.5f*ratio), bottomLeft.y + (y*ratio) + (0.5f*ratio)));
            }
        }
            ...
            for (int x = 0; x < countX; x++) {
                if (hits[x, y]) {
                    if (!previousWasHit) {
                        ...
                        currentInstance.transform.position = new Vector3(bottomLeft.x + (x*ratio) + (0.5f*ratio), bottomLeft.y + (y*ratio) + (0.5f*ratio), 0.0f);
                    } else {

                        // stretch prevois shadowCasterPrefab instance

                        currentInstance.transform.localScale = new Vector3(currentInstance.transform.localScale.x + (1.0f*ratio), (1.0f*ratio), 0.0f);
                        currentInstance.transform.Translate(new Vector3(0.5f*ratio, 0.0f, 0.0f));
                    }
            ...
            for (int x = 0; x < countX; x++) {
                ...
                if (bottomInstance != null && topInstance != null) {
                    if (bottomInstance != topInstance && bottomInstance.transform.localScale.x == topInstance.transform.localScale.x) {
                        ...
                        bottomInstance.transform.localScale = new Vector3(bottomInstance.transform.localScale.x, bottomInstance.transform.localScale.y + (1.0f*ratio), 0.0f);
                        bottomInstance.transform.Translate(new Vector3(0.0f, 0.5f*ratio, 0.0f));
    ...
    bool IsHit(Vector2 pos) {
        var margin = .2f*ratio*0; // prevents overlapping

        // get tile bounds

        var bottomLeft = new Vector2(pos.x - (0.5f*ratio) + margin, pos.y + (0.5f*ratio) - margin);
        var topRight = new Vector2(pos.x + (0.5f*ratio) - margin, pos.y - (0.5f*ratio) + margin);
